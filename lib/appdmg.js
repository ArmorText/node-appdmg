...
module.exports = exports = function (options) {
  ...
  pipeline.addStep('Fixing background', function(next) {
    ...
    var applicationName = global.opts.title
    var background = path.basename(global.bkgname)
    var iconSize = global.opts['icon-size'] || 80;
    var wp = {x: 0, y: 0}
    if (global.opts.window && global.opts.window.position) {
      wp = {x: global.opts.window.position.x, y: global.opts.window.position.y}
    }
    var ws = {w: 640, h: 480}
    if (global.opts.window && global.opts.window.size) {
      ws = {w: global.opts.window.size.width, h: global.opts.window.size.height}
    } else if (global.bkgsize) {
      ws = {w: global.bkgsize[0], h: global.bkgsize[1]}
    }
    var iconPosScript = global.opts.contents.map(e => {
      return `set position of item "${e.name || path.basename(e.path)}" of container window to {${e.x}, ${e.y}}`;
    }).join(`
        `)
    var disk = path.basename(global.temporaryMountPath)
    var execa = require('execa')
    execa('osascript', [], {input: `
    tell application "Finder"
      tell disk "${disk}"
        open
        set toolbar visible of container window to false
        set statusbar visible of container window to false
        set the bounds of container window to {${wp.x}, ${wp.y}, ${ws.w}, ${ws.h}}
